...
describe Nono do
  ...
  it "is valid without a yummly_code if the category is 'ingredient'" do
    nono = Nono.new(
      name: Faker::Name.name,
      category: 'ingredient')
    expect(nono).to be_valid
  ...
  it "is invalid without a yummly_code if the category is 'vegetarian'" do
    nono = Nono.new(category: "vegetarian", yummly_code: nil)
    nono.valid?
    expect(nono.errors[:yummly_code]).to include("can't be blank")
  end
